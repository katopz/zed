...
- [x] Automated `test_checkpoint_row_renders_with_show` (checkpoint-row Branch button code path with `checkpoint.show == true`; corrects prior analysis that this was blocked on git-backed checkpoint setup — `FakeFs` simulates git via `FakeGitRepositoryState`)
...
| `test_checkpoint_row_renders_with_show` | Pass — `FakeFs` + `.git` worktree + file-write during pending turn drives `checkpoint.show == true`; `to_markdown` shows "## User (checkpoint)"; full `ThreadView` render with `has_checkpoint_button == true` exercises the Branch button declaration without panic |
| `./script/clippy -p agent_ui` (after checkpoint-row test) | Clean (`--deny warnings`, plus `cargo-machete` + `typos`) |
| Manual GUI smoke | Fully automated — all 4 items covered |
...
| `crates/agent_ui/src/conversation_view.rs` | `test_prompt_jump_bookends_scroll_to_ends` (scroll wiring), `draw_real_thread_view` + `test_render_full_thread_view_smoke` (full render-path smoke), `test_checkpoint_row_renders_with_show` (checkpoint-row Branch button with `checkpoint.show == true`) |
...
| 2 | Checkpoint row shows both buttons | **Automated** | `test_checkpoint_row_renders_with_show` sets up a project with a `.git` worktree (`FakeFs` simulates git via `FakeGitRepositoryState`), starts a pending turn via `StubAgentConnection`'s `response_tx`/`end_turn` path, writes a file during the turn so `compare_checkpoints` detects a change and sets `checkpoint.show == true` (verified via `to_markdown` showing "## User (checkpoint)"), then renders the full `ThreadView` via `draw_real_thread_view` — exercising the checkpoint-row Branch button declaration under `.when(is_editable && has_checkpoint_button)` with `has_checkpoint_button == true`. `test_render_full_thread_view_smoke` additionally exercises the same render path for non-checkpoint entries. The branch-boundary logic the button triggers is covered by 5 unit tests in `branch_boundary_tests`. |
